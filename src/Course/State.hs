...
firstRepeat l =
  eval (findM
    (\a ->
      State $ \s ->
        (S.member a s, S.insert a s))
    l) S.empty
--firstRepeat Nil = Empty
--firstRepeat (x :. xs) =
  --if (findM (\a -> (lift2 (==) (Full a) (Full x))) xs) /= pure Empty then Full x else firstRepeat xs
...
isHappy ::
  ...
  -> Bool
isHappy x =
  (contains 1 . (flip eval) S.empty)
    (findM
      (\n ->
        State $ \s ->
          (n == 1 || S.member n s, S.insert n s))
      l)
  where
    l = (flip produce) x
          (\n ->
